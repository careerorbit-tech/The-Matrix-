...
- **Core Features:**
  - **Interactive "Tonight's Sky" Badge:** A dynamic lunar info badge in the Hero section providing real-time moon phase and illumination data.
  - **High-Fidelity Cosmic Animations:** Premium visual experiences like the `CosmicOrb` (3D-like glass effect with mouse tracking) and `ParticleBackground`.
  - **Local SEO Mastery:** Comprehensive JSON-LD structured data (Event, Organization, LocalBusiness) for dominance in Kolhapur search results.
  - **Dynamic Event Listings:** Responsive event cards with registration links (Google Forms) and image galleries.
  - **Cosmic Aesthetic:** A professional, dark-themed UI built with Framer Motion, Tailwind CSS v4, and Shadcn UI.
...
- **Key Configs:** 
  ...
  - Handles Cache-Control headers for images, favicons, and manifests to optimize performance (1 year immutable cache).
...
- **Premium Visuals:** 
  - **`MoonPhase.tsx`:** Renders the current moon phase with a precise SVG terminator shadow and atmospheric glow.
  - **`CosmicOrb.tsx`:** A complex, multi-layered glass orb animation with mouse-tracking tilt effects and dynamic particles.
  - **`ParticleBackground.tsx`:** A lightweight, performant star-field background.
...
- **Purpose:** The main landing page showcasing the club's identity.
- **Interactive Features:** 
  - **Tonight's Sky Badge:** A professional sliding badge that reveals detailed lunar data on hover.
  - **Parallax Imagery:** Smooth scroll-based transformations for the Hero nebula and telescope sections.
- **Execution Flow:** Hero (with interactive badge) -> Stats -> About Preview -> Upcoming Event Section -> ReviewSection -> Footer.
